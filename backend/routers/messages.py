...
from core.realtime import broadcast_to_conversation
...
async def send_message(body: SendMessageIn, current=Depends(get_current_user)):
    ...
    if body.plaintext_length and body.plaintext_length > 5000:
        raise HTTPException(413, "Message content too long")
    ...
    await broadcast_to_conversation(body.conversation_id, {"type": "message", "data": msg})
...
async def mark_read(body: MarkReadIn, current=Depends(get_current_user)):
    ...
    await db.message_reads.update_one(
        ...
        {"$set": {"last_read_message_id": up_to, "last_read_at": iso(now_utc())}},
        upsert=True,
    )
